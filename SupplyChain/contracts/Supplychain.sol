...
contract Procurement is owned{
   ...
   function shipNext(uint256 _itemid, address _nextStop) public
   {
       ...
       PresentConditionOfItem[_itemid].timestampShipped = block.timestamp;
   ...
   function updateStateOfArrivedItem(uint256 _itemid) public
   {
       ...
       SetPresentConditon(_itemid,msg.sender,block.timestamp,0,temp,PresentConditionOfItem[_itemid].vendor,PresentConditionOfItem[_itemid].quotation,false);
       History[_itemid].push(PresentConditionOfItem[_itemid]);
       SendMoney(_itemid);
       }
       else
       ...
       SetPresentConditon(_itemid,msg.sender,block.timestamp,0,temp,PresentConditionOfItem[_itemid].vendor,PresentConditionOfItem[_itemid].quotation,true);
       }
   ...
   function allotItemToVendor(uint256 _itemid,address payable _vendor,uint256 _quotation) public onlyowner
   {
       ...
       SetPresentConditon(_itemid,_vendor,block.timestamp,0,temp,_vendor,_quotation,true);
